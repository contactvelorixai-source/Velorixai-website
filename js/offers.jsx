...
function PlanCard({p}){
  ...
  return <div className="vx-lift" style={{position:"relative",display:"flex",flexDirection:"column",gap:"var(--space-lg)",minHeight:"100%",border:"1px solid "+(p.featured?"var(--border-translucent-strong)":"var(--color-hairline)"),borderRadius:14,padding:"clamp(22px,3vw,32px)",background:`linear-gradient(168deg, color-mix(in oklab, ${a1} ${p.featured?16:9}%, #101C38), #0B1630)`,"--glow":`color-mix(in oklab, ${a2} 60%, transparent)`,overflow:"hidden"}}>
    ...
    <div style={{display:"inline-flex",alignItems:"center",gap:9,alignSelf:"flex-start",padding:"7px 14px",borderRadius:999,border:`1px solid color-mix(in oklab, ${a2} 40%, transparent)`,background:`color-mix(in oklab, ${a1} 14%, transparent)`}}>
      ...
      <span style={{...mono,fontSize:11,color:"var(--color-ink)",whiteSpace:"nowrap"}}>{p.tokens}</span>
    </div>
    <ul style={{listStyle:"none",margin:0,padding:0,display:"grid",gap:"var(--space-md)"}}>
      <li style={{display:"flex",gap:11,alignItems:"flex-start"}}>
        <span style={{marginTop:2,color:"var(--brand-yellow)",flexShrink:0}}><OI name="check" size={15}/></span>
...
function Pricing(){
  ...
  return <Shell id="pricing" label="Pricing">
    <Reveal><Head eyebrow="PRICING" title="Pricing tailored to you, confirmed on your free demo." lead="Because every coaching or agency business is unique, we use your free demo to tailor a plan that matches your actual business size and app usage, with no generic pricing." max={780}/></Reveal>

    <Reveal delay={80} style={{marginTop:"clamp(36px,5vw,56px)"}}>
      <div className="price-range" style={{display:"grid",gridTemplateColumns:"auto 1fr auto",gap:"clamp(20px,3vw,40px)",alignItems:"center",border:"1px solid var(--color-hairline)",borderRadius:14,padding:"clamp(20px,3vw,28px) clamp(22px,3vw,32px)",background:"linear-gradient(120deg,#101C38,#0B1630)"}}>
    ...
    <Reveal delay={80} style={{marginTop:"clamp(28px,4vw,44px)",display:"grid",gap:"var(--space-lg)"}}>
      <ul style={{listStyle:"none",margin:0,padding:0,display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"var(--space-lg)"}} className="price-notes">
        {[["Plans scale with your business, from solo coach to full agency","layers","crm"],["Cancel or change your plan anytime after sign-up","refresh-cw","clients"],["No cost, no obligation. Just 30 minutes of your time.","clock","analytics"]].map(([t,ic,acc])=>
...
function Annual(){
  ...
  const dark="var(--color-ink)", line="var(--border-translucent)";
  return <Shell id="annual" label="Annual offer" tone="card">
    <Reveal>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:"var(--space-lg)"}}>
        <Tile icon="crown" size={38} accent="gold"/>
        <OEy color="var(--brand-yellow)">ANNUAL SUBSCRIPTION</OEy>
      </div>
      <Head title={`Go annual. Get more with ${S.brand.name}.`} lead={`Choose an annual ${S.brand.name} subscription and receive exclusive growth bonuses designed to help you establish your authority and digital presence.`}/>
    </Reveal>

    <Reveal delay={70} style={{marginTop:"clamp(32px,4.5vw,52px)"}}>
      <div className="annual-tokens" style={{display:"grid",gridTemplateColumns:"auto 1fr auto",gap:"clamp(20px,3vw,40px)",alignItems:"center",border:"1px solid "+line,borderRadius:14,padding:"clamp(20px,3vw,28px) clamp(22px,3vw,32px)",background:"linear-gradient(120deg,#101C38,#0B1630)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <Tile icon="coins" size={38} accent="gold"/>
          <span style={{...mono,color:"var(--color-body-mid)"}}>ANNUAL<br/>TOKENS</span>
        </div>
        <div>
          ...
          <div style={{...mono,color:"var(--color-body-mid)",marginTop:6,letterSpacing:"1px"}}>1,000 × 6 MONTHS + 500 × 6 MONTHS</div>
        </div>
        <div style={{display:"grid",gap:6,minWidth:150}}>
          {[["MONTHS 1-6","6,000",1],["MONTHS 7-12","3,000",0.5]].map(([l,v,w])=>
            <div key={l} style={{display:"grid",gap:4}}>
              <div style={{display:"flex",justifyContent:"space-between",gap:12,...mono,fontSize:10,color:"var(--color-body-mid)"}}><span style={{whiteSpace:"nowrap"}}>{l}</span><span style={{color:dark}}>{v}</span></div>
              <div style={{height:6,borderRadius:99,background:"rgba(255,255,255,.1)"}}><div style={{width:`${w*100}%`,height:"100%",borderRadius:99,background:"linear-gradient(90deg,var(--c-gold),var(--c-gold-soft))"}}></div></div>
            </div>)}
        </div>
      </div>
    </Reveal>

    <div style={{...mono,color:"var(--color-body-mid)",margin:"clamp(32px,4.5vw,52px) 0 var(--space-lg)"}}>CHOOSE ONE FREE BONUS</div>
    <div className="offer-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"clamp(24px,4vw,48px)",alignItems:"stretch"}}>
      {[["OPTION 1","Free professional website","A professionally designed website for your coaching, consulting or agency business.","globe","website",<SiteMock key="s"/>,null],
        ...
        <Reveal key={tag} className="vx-lift" style={{display:"flex",flexDirection:"column",gap:"var(--space-lg)",border:"1px solid "+line,borderRadius:14,padding:"clamp(18px,3vw,28px)",background:"linear-gradient(168deg,#101C38,#0B1630)","--glow":"rgba(255,215,0,.4)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <Tile icon={ic} size={36} accent={acc}/>
            <span style={{...mono,color:"var(--color-body-mid)"}}>{tag}</span>
            {badge?<span style={{...mono,fontSize:10,padding:"3px 10px",borderRadius:999,background:"rgba(255,215,0,.14)",border:"1px solid "+line,color:"var(--brand-yellow)",whiteSpace:"nowrap"}}>{badge}</span>:null}
          </div>
          ...
          <p className="vx-body-md" style={{margin:0,color:"var(--color-body)"}}>{copy}</p>
          <div style={{marginTop:"auto",paddingTop:"var(--space-sm)"}}>
            <button type="button" data-lead-cta className="vx-cta" style={{cursor:"pointer",font:"inherit",fontSize:14}}>{S.cta.book}</button>
          </div>
        </Reveal>)}
    </div>

    <div style={{display:"flex",gap:"var(--space-md)",alignItems:"center",flexWrap:"wrap",marginTop:"clamp(32px,5vw,56px)"}}>
      <button type="button" data-lead-cta className="vx-cta" style={{cursor:"pointer",font:"inherit",fontSize:14}}>{S.cta.annual}</button>
      <span style={{...mono,color:"var(--color-body-mid)"}}>ONE BONUS PER ANNUAL SUBSCRIPTION</span>
    </div>
  </Shell>;
}
