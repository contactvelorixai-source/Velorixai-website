...
const FAQS=[
  ["How much does VelorixAI cost?","Pricing depends on your plan: Prestige (8 current apps, quarterly) or Signature (16 current apps, annual). Our investment range is ₹25,900 to ₹85,900. We confirm exact pricing on your free demo call based on your niche, business size and the specific apps you need."],
      ...
      {TESTIMONIALS.map(([n,role,q],i)=>
        <Reveal key={n} delay={i*110} className="vx-lift" style={{border:"1px solid var(--color-hairline)",borderRadius:12,background:`linear-gradient(168deg, color-mix(in oklab, ${ACCENT[["crm","analytics","gold"][i%3]][0]} 12%, transparent), rgba(25,25,25,.6))`,"--glow":`color-mix(in oklab, ${ACCENT[["crm","analytics","gold"][i%3]][1]} 60%, transparent)`,padding:"var(--space-xl)",display:"grid",gap:"var(--space-lg)",alignContent:"start"}}>
          ...
          <div style={{display:"flex",gap:12,alignItems:"center",borderTop:"1px solid var(--color-hairline)",paddingTop:"var(--space-lg)"}}>
            <Tile size={36} accent={["crm","analytics","gold"][i%3]} radius={999}><span style={{fontFamily:"var(--font-mono)",fontSize:14,color:"var(--tile-fg)"}}>{n[0]}</span></Tile>
            <span>
...
function FinalCta(){
  return <Shell id="demo" label="Final CTA" style={{textAlign:"center"}}>
    ...
    <Reveal style={{position:"relative"}}>
      ...
      <p style={{...mono,color:"var(--color-body-mid)",marginTop:"var(--space-xl)"}}>No cost, no obligation. Just 30 minutes of your time.</p>
    </Reveal>
  </Shell>;
}
