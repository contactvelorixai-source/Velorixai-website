...
function PageHeader(){
  ...
  return <header style={{position:"sticky",top:0,zIndex:120,background:"rgba(10,10,10,0.9)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",borderBottom:"1px solid var(--color-hairline)"}}>
    <div style={{maxWidth:"var(--container-max)",margin:"0 auto",padding:"10px var(--space-xl)",display:"flex",alignItems:"center",gap:"var(--space-lg)"}}>
      <a href={S.links.home} aria-label={S.brand.name+", home"} style={{display:"inline-flex",alignItems:"center",gap:9,textDecoration:"none",color:"var(--color-ink)"}}>
        <img src={S.brand.logo} alt="" height="34" style={{display:"block",height:34,width:"auto",filter:"drop-shadow(0 0 12px rgba(255,215,0,.45))"}}/>
        <span className="vx-display-xs" style={{letterSpacing:"-0.3px",fontSize:19,whiteSpace:"nowrap"}}>{S.brand.name}</span>
      </a>
      <a href={S.links.home} className="vx-body-sm" style={{marginLeft:"auto",color:"var(--color-body)",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:8,whiteSpace:"nowrap"}}><PgI name="arrow-left" size={15}/>Back to site</a>
    </div>
  </header>;
}
  ...
  return <React.Fragment>
    ...
    <main style={{maxWidth:860,margin:"0 auto",padding:"clamp(48px,7vw,96px) var(--space-xl)"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:"var(--space-lg)"}}>
        ...
        <PgEy color="var(--brand-yellow)">{data.eyebrow}</PgEy>
      </div>
      ...
      <div style={{marginTop:"clamp(36px,5vw,60px)",display:"grid",gap:"clamp(28px,4vw,44px)"}}>
        {data.sections.map((s,i)=>
          <section key={s.h} style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"clamp(16px,3vw,28px)",paddingTop:"var(--space-xl)",borderTop:"1px solid var(--color-hairline)"}}>
            <span style={{fontFamily:"var(--font-mono)",fontSize:"clamp(18px,2.4vw,24px)",lineHeight:1,color:"var(--brand-yellow)",paddingTop:4}}>{String(i+1).padStart(2,"0")}</span>
            <div>
              ...
              <div style={{display:"grid",gap:"var(--space-md)"}}>
                ...
                {s.list?<ul style={{margin:0,paddingLeft:0,listStyle:"none",display:"grid",gap:"var(--space-sm)"}}>
                  {s.list.map(t=><li key={t} style={{display:"flex",gap:11,alignItems:"flex-start"}}>
                    <span style={{marginTop:3,color:"var(--brand-yellow)",flexShrink:0}}><PgI name="check" size={14}/></span>
      ...
      <div style={{marginTop:"clamp(40px,6vw,72px)",padding:"clamp(22px,3vw,32px)",borderRadius:14,border:"1px solid var(--color-hairline)",background:"linear-gradient(150deg,#101C38,#0B1630)",display:"grid",gap:"var(--space-md)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <Tile icon="mail" size={34} accent="email"/>
          <span style={{...mono,color:"var(--color-body-mid)"}}>QUESTIONS ABOUT THIS POLICY</span>
        </div>
        <p className="vx-body-md" style={{color:"var(--color-body)",margin:0}}>Write to <a href={"mailto:"+window.SITE.contact.email} style={{color:"var(--brand-yellow)"}}>{window.SITE.contact.email}</a> or message us on <a href={window.SITE.contact.whatsapp} target="_blank" rel="noopener" style={{color:"var(--brand-yellow)"}}>WhatsApp</a>.</p>
      </div>
    </main>
    <GlobalFooter/>
  </React.Fragment>;
